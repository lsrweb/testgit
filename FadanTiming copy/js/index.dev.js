...
var vm = new Vue({
	...
	mixins: [fanganMixin(),newCreatePreview()],
	data: {
		// fold:false,// 全部折叠开关
		send_time_zhuijia: "N",
		reply_msg_index_tmp: '',
		...
		tableSort: '',
		// 压缩图片
		compressDialog: false,		//自动压缩弹框
		compressLoading: false,		//压缩过程，大图可能会缓慢
		compressBefore64: '',		//被压缩64文件
		compressAfter64: '',		//压缩后64文件
		compressAfterSize: 0,		//压缩后64体积
		compressRank: 7,			//压缩等级
	},
	computed: {
		fold:{
		...
		allMessagesCollapsed: {
					get() {
						...
						return this.replyForm.msg.every(msg => msg.collapsed);
					},
					set(value) {
						this.replyForm.msg.forEach(msg => {
    ...
    if (this.replyForm.msg[index] != undefined) {
        ...
        try {
            ...
            console.error('Failed to parse reply_msg_index_tmp:', error);
            this.reply_msg_index_tmp = null;
        }

        try {
            this.reply_msg_tmp = JSON.parse(JSON.stringify(this.replyForm.msg[index]));
        } catch (error) {
            console.error('Failed to parse reply_msg_tmp:', error);
            this.reply_msg_tmp = null;
        }
		...
		addReplyMsg(index, action) {
			...
			that.replyForm.msg.splice(new_index, 0, msg_item);
			setTimeout(function() {
		...
		beforeAvatarUpload(file) {
            ...
            imgObj.onload = ()=>{    
                ...
                if(imgObj.width > 800 || !isLt2M){
					layer.close(that.load);
                    const reader = new FileReader();
                    reader.onload = (e) => { 
                        that.compressBefore64 = e.target.result;
                        // that.compressInedx.p_index = p_index;
                        // that.compressInedx.c_index = c_index;
                        that.compressRank = 7;  
                        that.compressImage();
                        that.compressDialog = true;
                    };
                    reader.onerror = (err) => {
                        if(!isLt2M){
                            that.$message({ message: '上传图片大超过2MB!浏览器版本过低不支持自动压缩', type: 'error', offset: 300 });
                        }else{
                            that.$message({ message: '上传图片宽度超过800像素!浏览器过低暂不支持自动压缩', type: 'error', offset: 300 });
                        }
                    };
		...
		compressImage() {
			var that = this;
			that.compressLoading = true;
			// 延迟10毫秒是为了防止64对象过大，导致页面渲染卡顿没渲染上loading
			setTimeout(() => {
				const img = new Image();
				img.src = that.compressBefore64;
				img.onload = () => {
				    const canvas = document.createElement('canvas');
				    const ctx = canvas.getContext('2d');
					canvas.width = img.width * (that.compressRank / 10);
					canvas.height = img.height * (that.compressRank / 10);
					...
					canvas.toBlob((blob) => {
						that.compressLoading = false;
						that.compressAfterSize = blob.size;
						that.compressAfter64 = canvas.toDataURL('image/png');
					}, 'image/png');
				};
				img.onerror = (err) => {
					that.compressLoading = false;
					that.$message({ message: '压缩失败，请手动压缩后上传', type: 'error', offset: 300 });
				};
			},200);
		},
		submitCompressImage(type){
			var that = this;
			if(type=='submit'){
				if((that.compressAfterSize / 1024 / 1024) > 10){
					that.$message({ message: '请保证压缩后体积在 10MB 以内！', type: 'error', offset: 300 });
				}else{
                    that.compressLoading = true;
					that.base64img(that.compressAfter64,0,0);
				}
			}else{
				that.compressDialog = false;
				that.compressLoading = false;
				that.compressBefore64 = '';
				that.compressAfter64 = '';
				that.compressAfterSize = '';
			}
		},
		//上传图片
        // base64img(url) {   
		...
		// 	setTimeout(() => {   
        //         let upData = {
        //             file:that.base64ToFile(url, 'image.png', 'image/png')
        //         }
        //         that.beforeAvatarRequest(upData,{type:'img'})
        //         return;
		// 	},20);
        // },
		beforeAvatarRequest(upData,mesData){
            var that = this;    
            $.post('/common/common/generateOssUploadUrl',{
                type:mesData.type,
                prefix: upData.file.name.split('.').pop(),
                path:'fenwei'
            },(json) => {
                if(json.code==200){
                    var uploadUrl = json.data.url;
                    var file = upData.file; 
                    var xhr = new XMLHttpRequest();
                    xhr.open('PUT','https://iyunzk.oss-cn-hangzhou.aliyuncs.com/'+ json.data.file_path +'?'+ json.data.urlParam, true);
                    xhr.setRequestHeader('Content-Type', json.data.content_type);
                    xhr.onload = () => {  
                        if (xhr.status === 200) {
                            console.log('文件上传成功:', json.data);
                            console.log(mesData,'data')
                            console.log(upData,'upData')
                            // let img_data = {
                            //     show_path:json.data.img_url,
                            //     save_path:json.data.file_path,
                            //     c_index:mesData.c_index,
                            //     p_index:mesData.p_index,
                            //     file_type:mesData.type,
                            //     old_name:upData.file.name,
                            //     file_size:upData.file.size,
                            // };
                            // that.afterUpimg(img_data);
                            // that.exUploadCDN();
                            // layer.close(that.load);
                            // that.upImageDialog = false;
                            // that.compressLoading = false;
                            // that.compressDialog = false;
                        } else {
                            that.$message({ message: '上传失败', type: 'error', offset: 300 , showClose: true});
                            console.log('文件上传失败:', xhr.statusText);
                            that.upImageDialog = false;
                        }
                    };
                    xhr.onerror = () => {
                    ...
                    that.$message({ message: json.msg, type: 'error', offset: 300,showClose: true });
                    that.upImageDialog = false;
                    layer.close(that.load)
                }
            })
		 ...
		 * 处理成功的上传
		 * 
		 * @param {Object} res  -服务器的响应对象
		 * @param {Object} file  -上传的文件对象
		 * @param {Object} item_msg  -与此上传关联的消息项
		 */
		handleAvatarSuccess_reply(res, file, item_msg) {
			const that = this;
			layer.close(that.load);
			if (res.code == '200') {
				var img_data = res.data;
				that.afterUpimg_new(img_data,item_msg);
		...
		afterUpimg_new(img_data,item_msg) {
			var that = this;
		...
		getPrevieweData(reply_msg, type) {
			...
			try {
				...
				console.log('JSON 解析失败，尝试 XML 解析')
				// JSON 解析失败，尝试 XML 解析
				try {
					// 处理可能没有XML声明的情况
		...
		saveVoice(type,sendType = false,row) {
			...
			$.post('/common/common/saveVoice', data, (json) =>{
				if(json.code == 200){
					if(type == 'edit'){
						that.collectionData.list[index].remark = that.collectionData.list[index].new_remark ;
						that.collectionData.list[index].edit_type = false;
						that.voiceSelet = '';
	...
	return {
		...
		methods: {
			getInitialDataForType(type) {
				switch (type) {
					case 'txt': return { msg: '' };
					case 'img': return { imageUrl: '', file: null };
			...
			onMessageTypeChange(messageItem) {
				...
				messageItem = {
					...messageItem,
					...this.getInitialDataForType(messageItem.type)
				};
			},
			getPreviewComponent(type) {
				const componentMap = {
					txt: 'preview-text-message',
			...
			addMessage(index, position, initialType = 'txt') {
				const newMessage = {
					id: 'msg_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9),
					msg_type_reply: initialType,
					...this.getInitialDataForType(initialType),
					delaySeconds: 1,
				...
				if (typeof index === 'undefined') { // Add to the end
					this.replyForm.msg.push(newMessage);
					newMsgIndex = this.replyForm.msg.length - 1;
				} else {
					if (position === 'above') {
						this.replyForm.msg.splice(index, 0, newMessage);
						newMsgIndex = index;
					} else if (position === 'below') {
						this.replyForm.msg.splice(index + 1, 0, newMessage);
						newMsgIndex = index + 1;
				...
				this.$nextTick(() => {
					if (newMsgIndex === this.replyForm.msg.length - 1 && (typeof index === 'undefined' || (position === 'below' && index === this.replyForm.msg.length - 2))) {
						this.scrollToBottom();
					} else {
			...
			scrollToElement(index) {
				this.$nextTick(() => {
					if (index < 0 || index >= this.replyForm.msg.length) {
						// console.error("ScrollToElement: Index out of bounds:", index);
						return;
					}
					const targetMessage = this.replyForm.msg[index];
					if (!targetMessage) {
